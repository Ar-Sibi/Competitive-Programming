...
int main(){
  ...
  // fur(i,0,libextd.size()){
  //   ll t=libextd[i][0];
  //   libextd[i][0]=libextd[i][1];
  //   libextd[i][1]=t;
  // }
  vvll answer(l);
  ...
  //   }
  // }
  vpll anshp(libonboard+1);
  fur(i,0,libonboard+1){
    anshp[i].X=answer[i].size();
    anshp[i].Y=i;
  }
  sort(anshp.begin(),anshp.end());
  vll extra(0);
  fur(i,0,anshp.size()){
    if(anshp[i].X<1){
      extra.pb(anshp[i].Y);
  ...
  fur(i,0,extra.size()){
    vll wt=libextd[(l-1)-i];
    libextd[(l-1)-i]=libextd[extra[i]];
    libextd[extra[i]]=wt;
  }
  //------------------------------------------------------------------------------
  answer=vvll(l,vll(0));
  unordered_map<ll,ll> visited1;
  libinwait=0;
  libonboard=-2;
  bookp=vll(l,0);
  fur(i,0,d){
    ...
    fur(j,0,libonboard+1){
      ...
      // cout<<libextd[j][2];
      while(c&&(bookp[j]<libextd[j][2])){
        if(!visited1[lsindex[libextd[j][3]][bookp[j]]]){
          answer[j].pb(lsindex[libextd[j][3]][bookp[j]]);
          visited1[lsindex[libextd[j][3]][bookp[j]]]=1;
          c--;
          // cout<<c<<"sadasdasd\n\n\n\n\n"<<lsindex[libextd[j][3]][bookp[j]]<<"\n\n\n";
        }
        bookp[j]++;
